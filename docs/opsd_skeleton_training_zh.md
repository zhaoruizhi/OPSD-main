...
### 1. 生成 full-train manifest

`prepare_sample_manifest.py` 当前没有单独的 `--all` 参数；可以把 `--sample-size` 设成大于数据集长度的数，它会返回全量 index。

```bash
cd /Users/zhaoruizhi/Desktop/code/OPSD-main

FULL_SKEL_OUT=/home/ruizzhao/OPSD-main/outputs/opsd_skeletons/qwen31b_full_train_$(date +%Y%m%d_%H%M%S)
...
python eval/prepare_sample_manifest.py \
  ...
  --sample-size 1000000 \
  --seed 0 \
  --output-file "$FULL_SKEL_OUT/sample_indices.json"
```

### 2A. 用本地 Qwen/vLLM 生成 skeleton

```bash
CUDA_VISIBLE_DEVICES=4 python eval/generate_semantic_skeletons.py \
  --dataset siyanzhao/Openthoughts_math_30k_opsd \
  --split train \
  --sample-indices-file "$FULL_SKEL_OUT/sample_indices.json" \
  ...
  --vllm-max-model-len 20000 \
  --vllm-top-p 1.0 \
  --vllm-top-k -1
```

### 2B. 用 OpenAI-compatible API 生成 skeleton
