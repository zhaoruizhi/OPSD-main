...
训练、KL 对比和 teacher 续写的可复制终端命令总表见 [OPSD 实验终端手册](experiment_runbook_zh.md)。

## 目标

这套改动给 OPSD 增加一个 `skeleton` teacher context 模式：student 仍然只看题目并进行 on-policy rollout，teacher 不再看完整 reference solution，而是只看 style-neutral semantic skeleton JSON。这样可以直接检验 skeleton 是否能替代完整 reference solution 作为 privileged signal。
...
- `--teacher_prompt_profile {current-style-neutral,legacy-20260629}`
  - `current-style-neutral`: 默认值，使用当前无答案的 style-neutral prompt。
  - `legacy-20260629`: 使用已由 2026-06-29 复刻实验验证的旧 prompt；在 skeleton block 后提供单独的 `Final answer:` 行。
- `--skeleton_file PATH`
  - skeleton 模式必传。
...
`SelfDistillationDataCollator` 接收 `teacher_context_mode` 和 `teacher_prompt_profile`。训练不再维护另一份 skeleton prompt，而是直接复用 rollout、两套 KL 和 teacher 续写共同使用的 profile-aware builder，避免不同实验链路的 prompt 漂移。
...
`skeleton + current-style-neutral` 模式的 teacher prompt 是：
...
- `Semantic Skeleton` block 里放的是 skeleton JSON。
- 两个 profile 都会从 skeleton JSON block 内移除 `final_answer` 字段。
- `current-style-neutral` 不单独注入答案；`legacy-20260629` 在 skeleton block 后注入 `Final answer: ...`。
- `legacy-20260629` 使用复数 JSON 字段 `checks`；`current-style-neutral` 使用单数 `check` 并附带字段解释。
- student prompt 完全不变。
- `reason_first=True` 目前只支持 `reference` 模式，不支持 skeleton 模式。

### `opsd_trainer.py`

`OPSDTrainer` 负责把 `teacher_context_mode` 和 `teacher_prompt_profile` 传给 collator，并保留 `semantic_skeleton`、`ground_truth` 这两个 dataset columns，避免 Trainer 自动裁掉它们。
...
脚本与 reference runner 一样支持：

- `TRAIN_GPU_IDS`：逗号分隔的物理 GPU 编号，例如 `4,5`。
- `NUM_PROCESSES`：Accelerate 进程数，通常等于选中的 GPU 数量。
- `MAIN_PROCESS_PORT`：多进程通信端口；并行启动多个实验时必须错开。
- `TEACHER_PROMPT_PROFILE`：选择 `current-style-neutral` 或 `legacy-20260629`。
- `RUN_CONFIG`：独立的训练输出目录名和 WandB run name 前缀。
- `MODEL_NAME_OR_PATH`、`OUTPUT_DIR`：覆盖模型和训练输出根目录。

每次训练启动时，rank 0 会在最终训练目录写入 `experiment_config.json`，其中包含解析后的 script/training/model 参数、实际命令行、CUDA/分布式环境、Git commit/dirty 状态以及 skeleton 文件路径和 SHA-256。

## 训练前准备：生成全量 skeleton

正式训练需要覆盖完整 train split，约 29.4k 条。复刻目录 `legacy_20260629_reproduction_20260721_171820/skeletons.jsonl` 只有 128 条，只能用于固定小样本 rollout/KL 实验，不能用于全量训练。
...
reference baseline 的训练根目录由脚本中的 `--output_dir` 指定，当前是：

```text
/home/ruizzhao/OPSD-main/outputs/opsd/
```
...
skeleton 的训练根目录由脚本中的 `--output_dir` 指定，当前是：
...
## `legacy-20260629 skeleton prompt 全量训练`

这个实验只复用已经验证过的 6 月 29 日 skeleton teacher prompt，不把 KL 复刻实验的 16384 rollout 上限套到训练。OPSD 训练继续保持脚本中的 `max_completion_length=1024`，其余学习率、LoRA、fixed teacher、sampling 和 loss 参数也保持不变。

### 1. 同步代码并检查全量 skeleton
...
```text
/home/ruizzhao/OPSD-main/outputs/opsd_quick/legacy_20260629_reproduction_20260721_171820/skeletons.jsonl
```

因为该文件只有 128 条，`--skeleton_subset_policy error` 会在训练开始前报告缺失的 problem IDs。
...
如果同一台服务器还有其他 accelerate 作业，把 `MAIN_PROCESS_PORT` 改成未被占用的端口。

### 3. 显式选择旧 prompt 和独立输出名
...
Training experiment config: .../experiment_config.json
Skeleton subset policy: error
Training rows: <完整行数> / <完整行数>
```
...
- `semantic_skeleton` 会被 JSON dump 后放入 `Semantic Skeleton` block。
- `final_answer` 从 JSON 中移除，也不会通过单独的 `Final answer: ...` 行注入。
